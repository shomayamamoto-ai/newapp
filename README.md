...
- **競合の保存数・視聴維持率・平均視聴時間** — 各社のInsights APIは自社アカウント
  の投稿にしか開放されていません。自社投稿側（PDCA）では取得できます。
- **TikTokの競合データ全般** — 公開キーワード検索APIが存在しません。CSV取り込み
  （`snsauto research import`）で手動収集したデータは扱えます。
- **どの音源がトレンドか** — 上記の通り。
